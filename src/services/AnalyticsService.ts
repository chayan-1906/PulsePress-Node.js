...
const updateSourceAnalytics = async ({source, action, readingTime = 0}: UpdateSourceAnalyticsParams): Promise<UpdateSourceAnalyticsResponse> => {
    try {
        if (!source) {
        ...
        if (!existingAnalytics) {
            const newAnalytics = await createInitialAnalytics(source, action, readingTime);
            return {sourceAnalytics: newAnalytics, isUpdated: true};
        }

        const updatedAnalytics = await updateExistingAnalytics(existingAnalytics, action, readingTime);
...
const createInitialAnalytics = async (source: string, action: string, readingTime: number): Promise<ISourceAnalytics> => {
    const initialData = {
        source,
...
const updateExistingAnalytics = async (analytics: ISourceAnalytics, action: string, readingTime: number): Promise<ISourceAnalytics> => {
    const updates: any = {};
...
const calculateEngagementScore = (metrics: any): number => {
...
const getSourceAnalytics = async ({limit = 20, sortBy = 'engagementScore', sortOrder = 'desc'}: GetSourceAnalyticsParams): Promise<GetSourceAnalyticsResponse> => {
    try {
...
const getTopPerformingSources = async ({limit = 10, minViews = 10}: GetTopPerformingSourcesParams): Promise<GetTopPerformingSourcesResponse> => {
    try {
...
export {updateSourceAnalytics, getSourceAnalytics, getTopPerformingSources};
